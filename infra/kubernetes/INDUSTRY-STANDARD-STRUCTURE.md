...
touch infra/kubernetes/base/{namespace,configmap,secrets,deployment,service,ingress,kustomization}.yaml

# Create all overlay files
touch infra/kubernetes/overlays/{dev,staging,prod}/kustomization.yaml
touch infra/kubernetes/overlays/{dev,staging,prod}/configmap-patch.yaml
touch infra/kubernetes/overlays/{dev,staging,prod}/deployment-patch.yaml
touch infra/kubernetes/overlays/{dev,staging,prod}/namespace-patch.yaml

# Create production-specific files
touch infra/kubernetes/overlays/prod/{hpa,rbac,network-policy}.yaml

# Create scripts
touch infra/kubernetes/scripts/{deploy,rollback,health-check}.sh
chmod +x infra/kubernetes/scripts/*.sh
```

### **2. Update CI/CD Pipeline:**
- Update GitHub Actions workflow
- Point to correct infra/kubernetes directory
